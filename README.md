...
\begin{align*}
    ...
    u(x) &= g(x) \quad\text{on}\quad \Omega_{Dirichlet}, \\
...
$$
\hat{u}(x_k) =
\frac{P^B(x_k, x_{k+1})\hat{u}(x_{k+1})}{\alpha(x_k)p^{\partial \mathrm{St}(x_k,r)}(x_{k+1})} - \frac{G^B(x_k, z_{k+1})h(z_{k+1})} {\alpha(x_k)p^{\partial \mathrm{St}_N(x_k,r)}(z_{k+1})} + 
\frac{G^B(x_k, y_{k+1})f(y_{k+1})}{\alpha(x_k)p^{\mathrm{St}(x_k,r)}(y_{k+1})}.
$$
